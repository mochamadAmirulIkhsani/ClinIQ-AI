...
describe('quiz API', () => {
   ...
   it('reveal clue requires attempt_id', async () => {
      const response = await authPost('/api/v1/quiz/reveal-clue', {})

      expect(response.status).toBe(400)
      expect(response.body.success).toBe(false)
      expect(response.body.message).toBe('attempt_id is required')
   })

   it('reveal clue rejects attempt owned by another user', async () => {
   ...
   it('reveal clue reveals clue 2 after clue 1', async () => {
      const attempt = await createAttempt(users.primary, quizData.first.vignette, {
         clues_revealed: 1
      })
      ...
      expect(response.body.success).toBe(true)
      expect(response.body.data.clues_revealed).toBe(2)
      expect(response.body.data.clue).toMatchObject({
         clue_number: 2,
         content: 'quiz-test-a clue 2'
      })
   })

   it('reveal clue reveals clue 3 after clue 2', async () => {
      const attempt = await createAttempt(users.primary, quizData.first.vignette, {
         clues_revealed: 1
      })

      await authPost('/api/v1/quiz/reveal-clue', {
         attempt_id: attempt.id
      })
      ...
      expect(response.status).toBe(200)
      expect(response.body.data.clues_revealed).toBe(3)
      expect(response.body.data.clue).toMatchObject({
         clue_number: 3,
         content: 'quiz-test-a clue 3'
      })
   })

   it('reveal clue rejects reveal after max clues', async () => {
      const attempt = await createAttempt(users.primary, quizData.first.vignette, {
         clues_revealed: 5
      })
      ...
      expect(response.status).toBe(400)
      expect(response.body.success).toBe(false)
      expect(response.body.message).toBe('All clues already revealed')
   })

   it('reveal clue rejects completed attempt', async () => {
      const attempt = await createAttempt(users.primary, quizData.first.vignette, {
         is_correct: true,
         score: 500
      })
